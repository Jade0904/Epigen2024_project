...
DNase_peaks <- list.files("data/peaks", pattern = "^DNase_", full.names = TRUE) %>% lapply(rtracklayer::import, format="narrowPeak")
DNase_peaks <- reduce(unlist(GRangesList(DNase_peaks)), with.revmap=TRUE)

GR_peaks <- list.files("data/peaks", pattern = "^GR_", full.names = TRUE) %>% lapply(rtracklayer::import, format="narrowPeak")
GR_peaks <- reduce(unlist(GRangesList(GR_peaks)), with.revmap=TRUE)

JUN_peaks <- list.files("data/peaks", pattern = "^JUN_", full.names = TRUE) %>% lapply(rtracklayer::import, format="narrowPeak")
JUN_peaks <- reduce(unlist(GRangesList(JUN_peaks)), with.revmap=TRUE)

JUNB_peaks <- list.files("data/peaks", pattern = "^JUNB_", full.names = TRUE) %>% lapply(rtracklayer::import, format="narrowPeak")
JUNB_peaks <- reduce(unlist(GRangesList(JUNB_peaks)), with.revmap=TRUE)

# we intersect the following results from differential analysis with the set of peaks from each assay
...
# create summarized experiment object
# we still use consensus peaks here because the peaks for GR are missing from the original paper supplementary materials, and we decide not to run peak calling process for simplicity
